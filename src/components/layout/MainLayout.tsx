      ...
      <aside className="w-60 shrink-0 bg-white border-r border-slate-200 flex flex-col">
        <div className="h-16 flex items-center px-6 border-b border-slate-200">
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-bold text-slate-900 text-lg">Preproute</span>
          </div>
        </div>

        <nav className="flex-1 px-3 py-4">
          {[{ path: '/dashboard', label: 'Dashboard', icon: LayoutDashboard }].map((item) => {
        ...
        <div className="p-3 border-t border-slate-200">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <button onClick={handleLogout} className="btn-icon text-slate-400 hover:text-red-500 hover:bg-red-50" title="Logout">
              <LogOut size={15} />
            </button>
          </div>
        </div>
